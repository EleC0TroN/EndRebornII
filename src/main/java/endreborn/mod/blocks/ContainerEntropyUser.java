...
import endreborn.mod.gui.SlotEFuel;
import endreborn.mod.gui.SlotEOut;
...
public class ContainerEntropyUser extends Container
{
    private final TileEntropyUser tileentity;
    private int cookTime, totalCookTime, burnTime, currentBurnTime;

    public ContainerEntropyUser(InventoryPlayer player, TileEntropyUser tileentity)
    {
    ...
    public ItemStack transferStackInSlot(EntityPlayer playerIn, int index)
    {
        ...
        if(slot != null && slot.getHasStack())
        {
            ...
            else if(index != 2 && index != 1 && index != 0)
            {
                ...
                if(!RecipesUser.getInstance().getAltarResult(stack1, slot1.getStack()).isEmpty())
                {
                    if(!this.mergeItemStack(stack1, 0, 2, false))
                    {
                        return ItemStack.EMPTY;
                    }
                    else if(TileEntropyUser.isItemFuel(stack1))
                    {
